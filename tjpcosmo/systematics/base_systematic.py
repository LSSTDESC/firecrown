...
class Systematic(object):
    ...
    def __init__(self, name, **config):
        ...
        self.values = {}
        print(
            f"Would now create systematic "
            f"{self.__class__.__name__} from config info: {config}")
    ...
    def from_info(cls, name, config):
        ...
        if class_name is None:
            raise ValueError(
                "Systematic is missing 'type' entry in param file")
...
class SourceSystematic(Systematic):
    ...
    def adjust_requirements(self, source):
        if (len(self.modified_source_properties) == 0):
            ...
            print(
                f"Systematic {self.__class__.__name__} "
                "does not modify any source properties!")
            print(
                f"If this is unintended, register them in "
                f"{self.__class__.__name__}.modified_source_properties")

        # print(f"required {self.required_source_properties}")
        # print(f"existing {source.eval_source_prop}")
        if (set(self.required_source_properties) <=
                set(source.eval_source_prop)):
