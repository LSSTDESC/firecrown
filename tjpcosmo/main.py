...
parser.add_argument(
    '--list-samplers',
    action='store_true',
    help='List all available sampling methods.')
parser.add_argument(
    "yaml_config_file",
    help="Input YAML configuration file describing run")
parser.add_argument(
    "--mpi",
    action='store_true',
...
parser.add_argument(
    ...
    default=0,
    help=(
        "Run with the given number of processes in shared memory "
        "multiprocessing (this is experimental and does not work "
        "for multinest)."))
parser.add_argument(
    "--pdb",
    action='store_true',
    help=(
        "Start the python debugger on an uncaught "
        "error. Only in serial mode."))
...
def main(args):
    ...
    Reads in the command line argument, which should be the .ini file used
    for this run. Look at example for that Params.ini for how one of these
    are structured.

    Cosmosis calls the theory_model.py for information on how to execute
    the TJPCosmo code.
    """
    ...
    # to configure cosmosis and its samplers, and once in the
    # cosmosis_entry_point file to configure the module and calculation
    ...
    # Run the cosmosis main function, optionally under two
    ...
    # or just in serial, with the error handling
        ...
        try:
            ...
            if args.pdb:
                print("pdb")
                print(
                    "There was an exception - starting python debugger "
                    "because you ran with --pdb")
                print(error)
                pdb.post_mortem()
            else:
                print("here")
