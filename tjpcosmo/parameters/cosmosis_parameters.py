...
def block_to_parameters(block, consistency):
    """This fucntion translates the parameters from a cosmosis block to
    TJPCosmo's Parameter class (A sub class of a dictionary. This list should
    be consistent with the DESC standard for cosmological parameters. A few
    parameters are mandatory while others will be getting defaults values if
    not specified in the initial file.
    """
    ...
    # Omega_n_mass = block.get_double(
    #     names.cosmological_parameters, 'Omega_n_mass', 0.0)
    Omega_n_rel = block.get_double(
        names.cosmological_parameters, 'Omega_n_rel', 0.0)
    Omega_g = block.get_double(
        names.cosmological_parameters, 'Omega_g', 0.0)
    N_nu_mass = block.get_double(
        names.cosmological_parameters, 'N_nu_mass', 0.0)
    N_nu_rel = block.get_double(
        names.cosmological_parameters, 'N_nu_rel', 3.046)
    mnu = block.get_double(names.cosmological_parameters, 'mnu', 0.0)

    # Now if we have provided the code with Omega_k or Omega_l
    # it will figure out
    known_parameters = {}
    ...
    return parameters
    # Everything done so far gets thrown into the to DESC standard
    # cosmoogy base.
    # cosmology = CosmoBase(Omega_c, Omega_b, Omega_l, h, n_s, A_s, sigma_8,
    #                       Omega_g, Omega_n_mass, Omega_n_rel, w0, wa,
    #                       N_nu_mass, N_nu_rel, mnu)
