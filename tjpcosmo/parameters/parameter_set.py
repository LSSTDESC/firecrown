...
class ParameterSet(OrderedDict):
    """A class for holding the names and current values of the parameters that
    will be varied.

    Typically, these are the parameters that are stepped over in a Markov chain
    or similar code.  This class is little more than a glorified Python dict
    with a bit of extra syntactic sugar to let parameters be accessed as
    attributes rather than keys.

    A ParameterSet object is constructed with arbitrary kwargs, giving the
    names of the parameters to include.  The value of each should be either a
    type (float, int or bool) or a value with one of these types.

    Example:

        >>> cosmo_params = ParameterSet(Omega_m=float, h=float, Lambda=float,
                                        As=float, ns=float, ...
                                        sum_neutrino_masses=float,
                                        n_neutrino_species=int, ...
                                        inverted_neutrino_hierarchy=bool)
        >>> ia_params = ParameterSet(a1=1.4, a2=2.3)

    In addition, the value of an argument may be another ParameterSet object,
    in which case the parameters will be accessed hierarchically (not unlike
    ...
    You can set or access hierarchical parameter values with the same syntax as
    a dict or with a single key string using a '.' to separate the keys in the
    different levels:
        ...
        for key, value in kwargs.items():
            if isinstance(value, ParameterSet):
                ...
                if value not in self._valid_types:
                    raise ValueError(
                        "Type %s not supported for parameter %s" % (
                            value, key))
    ...
    def _check_type(self, key, value, valid_types):
        # If value has an invalid type, see if it is convertible into either
        # int or float.
        if type(value) not in valid_types:
            # If not one of these, see if it is implicitly convertible to
            # int or float.
            try:
                if int in valid_types and int(value) == value:
                    return int(value)
                if float in valid_types and float(value) == value:
                    return float(value)
            except (TypeError, ValueError):
                pass
        if type(value) not in self._valid_types:
            raise ValueError(
                "Type %s not supported for parameter %s" % (type(value), key))
        return value
    ...
    def __setitem__(self, key, value):
        if key in self:
            ...
            if type(value) != value_type:
                raise ValueError(
                    "When setting %s, expected a %s. Got %s" % (
    ...
    # The str representation isn't valid syntax, since the keys may
    # have dots in them,
