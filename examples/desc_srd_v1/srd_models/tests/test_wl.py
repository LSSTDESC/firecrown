...
def test_compute_red_frac_z_Az_redfrac():
    ...
    vals = [
        _compute_red_frac_z_Az(
            _z, ccl.luminosity_distance(COSMO, 1 / (1.0 + _z)), beta_ia, lpiv_beta_ia
        )
        for _z in z
    ]
    rf, _ = zip(*vals)
...
def test_ia_bias():
    ...
    params = {"a": eta_ia, "b": eta_ia_highz, "c": beta_ia}

    sys = KEBNLASystematic(eta_ia="a", eta_ia_highz="b", beta_ia="c")
    ...
    fac *= 1.0 * (src.z_ < 0.7) + np.power((1 + src.z_) / (1 + 0.7), eta_ia_highz) * (
        src.z_ > 0.7
    )
    fac *= az
    fac *= rf
