...
def add_srci_lensj_ell_cl(sacc_data, i, j, mean_z, cl):
    ...
    lmax_lens = (
        LENS_KMAX * ccl.comoving_radial_distance(COSMO, 1.0 / (1.0 + mean_z)) - 0.5
    )
    lmax = min(lmax_lens, SRC_LMAX)
    msk = (ELL_VALUES < lmax) & (cl > 1e-15)
    assert len(ELL_VALUES) == len(cl)
    sacc_data.add_ell_cl(
        "galaxy_shearDensity_cl_e", "src%d" % i, "lens%d" % j, ELL_VALUES[msk], cl[msk]
    )
    return msk
...
def add_lensi_lensi_ell_cl(sacc_data, i, mean_z, cl):
    ...
    lmax = LENS_KMAX * ccl.comoving_radial_distance(COSMO, 1.0 / (1.0 + mean_z)) - 0.5
    msk = (ELL_VALUES < lmax) & (cl > 1e-15)
    assert len(ELL_VALUES) == len(cl)
    sacc_data.add_ell_cl(
        "galaxy_density_cl", "lens%d" % i, "lens%d" % i, ELL_VALUES[msk], cl[msk]
    )
    return msk
...
def add_srci_srcj_ell_cl(sacc_data, i, j, cl):
    ...
    assert len(ELL_VALUES) == len(cl)
    sacc_data.add_ell_cl(
        "galaxy_shear_cl_ee", "src%d" % i, "src%d" % j, ELL_VALUES[msk], cl[msk]
    )
    return msk
...
def add_src_tracers(sacc_data):
    ...
    interp = scipy.interpolate.interp1d(
        cuml_dndz_true, zarr, fill_value="extrapolate", kind="cubic"
    )
...
def _make_pz(z_true_min, z_true_max, _pz, _sigmaz, n_z=1000):
    ...
    for i, _z in enumerate(z_obs):
        ...
        def _func(z):
            return (
                _pz(z) * np.exp(-0.5 * np.power((z - _z) / _sigmaz(z), 2)) / _sigmaz(z)
            )
