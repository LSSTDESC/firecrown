...
integ_options = [Ncm.IntegralndMethod.P,Ncm.IntegralndMethod.P_V,Ncm.IntegralndMethod.H,Ncm.IntegralndMethod.H_V]
for integ_method in integ_options:
    abundance_test = ClusterAbundance(hmd_200, hmf_name, hmf_args, integ_method = integ_method)
    test_m_list = []
    t1 = time.time()
    for i in range(0,3):
        for j in range(0,4):
    ...
    print(f"The time for {integ_method} is {t2-t1}\n\n The counts value is {test_m_list}\n\n")
