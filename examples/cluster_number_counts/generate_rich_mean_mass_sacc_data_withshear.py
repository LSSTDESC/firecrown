...
def generate_cosmo(H0, Ob0, Odm0, n_s, sigma8):
    ...
    cosmo_ccl = ccl.Cosmology(
        Omega_b=Ob0, Omega_c=Odm0, sigma8=sigma8,
        w0=-1, wa=0, h=H0/100.0, n_s=n_s
    )
...
def generate_cluster_data(cosmo, area, M0, z0, lnRl, lnRu, zl, zu):
    ...
    hmf = Nc.HaloMassFunction.new(
        Nc.Distance.new(2.0),
        Ncm.PowspecFilter.new(Nc.PowspecMLTransfer.new(Nc.TransferFuncEH.new()),
...
def compute_profiles(N_z, N_rich, cosmo_ccl, z, richness, logM):
    ...
    radius_centers = [np.mean(radius_edges[i:i+2]) for i in range(len(radius_edges)-1)]
    ...
    for zc, lM in zip(z, logM):
        ...
        # reduced shear g_t
        beta = compute_beta_s_mean_from_distribution(z_cl=zc, z_inf=10,
                                                     cosmo=moo.cosmo, zmax=5.0)
        beta2 = compute_beta_s_square_mean_from_distribution(z_cl=zc, z_inf=10,
                                                             cosmo=moo.cosmo, zmax=5.0)
    ...
    return (counts, mean_logM, mean_dsigma, mean_gt,
            z_edges, r_edges, radius_edges, cov)
...
def generate_sacc_file():
    ...
    z, richness, logM = generate_cluster_data(cosmo, area, M0, z0,
                                              lnRl, lnRu, zl, zu)

    (counts, mean_logM, mean_dsigma, mean_gt,
     z_edges, r_edges, R_edges, cov) = compute_profiles(
        N_z, N_richness, cosmo_ccl, z, richness, logM
    )
    ...
    # Fill counts and logM
    for c, (zlab, rlab) in zip(counts.flatten(),
                               itertools.product(z_labels, r_labels)):
        s.add_data_point(t_counts, (survey, zlab, rlab), int(c))

    for m, (zlab, rlab) in zip(mean_logM.flatten(),
                               itertools.product(z_labels, r_labels)):
        s.add_data_point(t_logmass, (survey, zlab, rlab), m)

    # Fill DeltaSigma and reduced shear
    for zi, zlab in enumerate(z_labels):
        for ri, rlab in enumerate(r_labels):
            for iR, Rlab in enumerate(R_labels):
                s.add_data_point(
                    t_dsigma,
                    (survey, zlab, rlab, Rlab),
                    mean_dsigma[zi,ri,iR]
                )
    for zi, zlab in enumerate(z_labels):
        for ri, rlab in enumerate(r_labels):
            for iR, Rlab in enumerate(R_labels):
