...
def build_likelihood(
    ...
    if build_parameters.get_bool("use_mean_shear", False):
        average_on |= ClusterProperty.SHEAR
    print("building LIKE")
    print("average_on:", average_on)
    print(ClusterProperty.COUNTS in average_on)
    print(ClusterProperty.SHEAR in average_on)

    survey_name = "numcosmo_simulated_redshift_richness"
    _likelihoods = [BinnedClusterNumberCounts(average_on, survey_name, MurataBinnedSpecZRecipe())]
    if average_on & ClusterProperty.SHEAR:
        _likelihoods += [BinnedClusterNumberCounts(average_on, survey_name, MurataBinnedSpecZRecipe())]
