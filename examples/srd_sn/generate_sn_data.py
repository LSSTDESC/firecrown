...
def read_hubble_data(args):
    ...
    if len(args) == 4:
        path = args[1]
        hubble_filename = args[2]
        covariance_filename = args[3]
        y1dat = np.loadtxt(f"{path}/{hubble_filename}", comments="#", dtype=str)
        y1cov = np.loadtxt(f"{path}/{covariance_filename}", comments="#", unpack=True)
        ...
        if os.path.exists(dirname_year1):
            ...
            ensure_local_data_files(dirname_year10, url)
