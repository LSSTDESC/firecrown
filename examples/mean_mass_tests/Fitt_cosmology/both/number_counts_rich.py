...
def build_likelihood(_):
    ...
    stats = ClusterNumberCounts(
        "cluster_counts_richness_proxy_plusmean", "cluster_mass_count_wl", massfunc
    )
    stats = [stats]
    """
    ...
    saccfile = os.path.expanduser(
        os.path.expandvars(
            "${FIRECROWN_DIR}/examples/mean_mass_tests/Fitt_cosmology/both/clusters_numcosmo_richness_plusmean_data.sacc"
        )
    )
