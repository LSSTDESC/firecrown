...
def build_likelihood(_):
    ...
    stats = ClusterNumberCounts(
        "cluster_counts_richness_meanonly_proxy", "cluster_mass_count_wl", massfunc
    )
    stats = [stats]
    """
    ...
    saccfile = os.path.expanduser(
        os.path.expandvars(
            "${FIRECROWN_DIR}/examples/mean_mass_tests/Fitt_cosmology/mean_mass/clusters_numcosmo_richness_meanonly_data.sacc"
        )
    )
