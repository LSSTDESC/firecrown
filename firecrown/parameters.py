...
class DerivedParameterCollection:
    ...
    def __getitem__(self, index):
        """Gets section, name, value tuple for each element in the collection."""
        required_parameter = list(self.derived_parameters.values())[index]
        return (
            required_parameter.section,
            required_parameter.name,
            required_parameter.get_val(),
        )

    def add_required_parameter(self, derived_parameter: DerivedParameter):
        """Adds derived_parameter to the collection, it raises an ValueError if a required parameter with the same
        name is already present in the collection."""
