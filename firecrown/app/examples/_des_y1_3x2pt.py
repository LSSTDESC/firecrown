...
"""
    ...
    def _get_ccl_cosmo_params(self) -> Model:
        return Model(
            ...
            parameters=[
                Parameter.from_tuple("Omega_c", r"\Omega_{c}", 0.20, 0.35, 0.25, True),
                Parameter.from_tuple("Omega_b", r"\Omega_{b}", 0.03, 0.06, 0.05, True),
                Parameter.from_tuple("h", r"h", 0.60, 0.80, 0.73, False),
                Parameter.from_tuple("n_s", r"n_{s}", 0.90, 1.10, 0.96, False),
                Parameter.from_tuple("Omega_k", r"\Omega_{k}", -0.2, 0.2, 0.0, False),
                Parameter.from_tuple("w0", r"w_{0}", -1.5, -0.5, -1.0, False),
                Parameter.from_tuple("wa", r"w_{a}", -1.0, 1.0, 0.0, False),
                Parameter.from_tuple("T_CMB", r"T_{CMB}", 2.7, 2.8, 2.725, False),
                Parameter.from_tuple("Neff", r"N_{eff}", 3.0, 4.0, 3.046, False),
                Parameter.from_tuple("m_nu", r"m_{\nu}", 0.0, 0.1, 0.0, False),
                Parameter.from_tuple("sigma8", r"\sigma_{8}", 0.7, 0.9, 0.8, False),
            ],
        )
    ...
    def get_models(self) -> list[Model]:
        ...
        params_tatt: list[tuple[str, str, float, float, float, bool]] = [
            ...
            ("ia_a_2", r"A_{\mathrm{IA},2}", 0.4, 0.6, 0.5, True),
            ("ia_a_d", r"A_{\mathrm{IA},d}", 0.4, 0.6, 0.5, True),
            ("ia_zpiv_1", r"z_{\mathrm{piv},1}", 0.0, 1.0, 0.62, False),
            ("ia_zpiv_2", r"z_{\mathrm{piv},2}", 0.0, 1.0, 0.62, False),
            ("ia_zpiv_d", r"z_{\mathrm{piv},d}", 0.0, 1.0, 0.62, False),
            ("ia_alphaz_1", r"\alpha_{z,1}", -5.0, 5.0, 0.0, False),
            ("ia_alphaz_2", r"\alpha_{z,2}", -5.0, 5.0, 0.0, False),
            ("ia_alphaz_d", r"\alpha_{z,d}", -5.0, 5.0, 0.0, False),
            ("src0_delta_z", r"\delta z_0", -0.05, 0.05, 0.0, True),
        ]
        params_hmia: list[tuple[str, str, float, float, float, bool]] = [
            ("ia_a_1h", r"A_{\mathrm{IA},1h}", 0.0, 0.01, 0.001, True),
            ("ia_a_2h", r"A_{\mathrm{IA},2h}", 0.0, 2.0, 1.0, True),
            ("src0_delta_z", r"\delta z_0", -0.05, 0.05, 0.0, True),
        ]
        params_pk_mod: list[tuple[str, str, float, float, float, bool]] = [
            ("f_bar", r"f_{\mathrm{bar}}", 0.0, 1.0, 0.5, True),
            ("src0_delta_z", r"\delta z_0", -0.05, 0.05, 0.0, True),
        ]
        params_yaml_mu_sigma: list[tuple[str, str, float, float, float, bool]] = [
            ("mg_musigma_mu", r"\mu_{\mathrm{MG}}", 0.8, 1.2, 1.0, True),
            ("mg_musigma_sigma", r"\sigma_{\mathrm{MG}}", 0.8, 1.2, 1.0, True),
            ("mg_musigma_c1", r"c_1", 0.8, 1.2, 1.0, False),
            ("mg_musigma_c2", r"c_2", 0.8, 1.2, 1.0, False),
            ("mg_musigma_lambda0", r"\lambda_0", 0.8, 1.2, 1.0, False),
            ("ia_bias", r"\beta_{\mathrm{ia}}", -5.0, 5.0, 0.5, True),
            ("alphaz", r"\alpha_z", -5.0, 5.0, 0.0, True),
            ("z_piv", r"z_{\mathrm{pivot}}", 0.0, 1.0, 0.62, False),
            ("lens0_bias", r"b_0", 0.8, 3.0, 1.4, True),
            ("lens1_bias", r"b_1", 0.8, 3.0, 1.6, True),
            ("lens2_bias", r"b_2", 0.8, 3.0, 1.6, True),
            ("lens3_bias", r"b_3", 0.8, 3.0, 1.9, True),
            ("lens4_bias", r"b_4", 0.8, 3.0, 2.0, True),
            ("lens0_delta_z", r"\delta z_0", -0.05, 0.05, 0.001, True),
            ("lens1_delta_z", r"\delta z_1", -0.05, 0.05, 0.002, True),
            ("lens2_delta_z", r"\delta z_2", -0.05, 0.05, 0.001, True),
            ("lens3_delta_z", r"\delta z_3", -0.05, 0.05, 0.003, True),
            ("lens4_delta_z", r"\delta z_4", -0.05, 0.05, 0.000, True),
            ("src0_mult_bias", r"m_0", 0.001, 0.012, 0.012, True),
            ("src1_mult_bias", r"m_1", 0.001, 0.012, 0.012, True),
            ("src2_mult_bias", r"m_2", 0.001, 0.012, 0.012, True),
            ("src3_mult_bias", r"m_3", 0.001, 0.012, 0.012, True),
            ("src0_delta_z", r"\delta z_0", -0.05, 0.05, -0.001, True),
            ("src1_delta_z", r"\delta z_1", -0.05, 0.05, -0.019, True),
            ("src2_delta_z", r"\delta z_2", -0.05, 0.05, 0.009, True),
            ("src3_delta_z", r"\delta z_3", -0.05, 0.05, -0.018, True),
        ]
        params_std: list[tuple[str, str, float, float, float, bool]] = [
            ("ia_bias", r"\beta_{\mathrm{ia}}", -5.0, 5.0, 0.5, True),
            ("alphaz", r"\alpha_z", -5.0, 5.0, 0.0, True),
            ("z_piv", r"z_{\mathrm{pivot}}", 0.0, 1.0, 0.62, False),
            ("lens0_bias", r"b_0", 0.8, 3.0, 1.4, True),
        ...
        params_pt: list[tuple[str, str, float, float, float, bool]] = [
            ("ia_a_1", r"A_{\mathrm{IA},1}", 0.9, 1.2, 1.0, True),
            ("ia_a_2", r"A_{\mathrm{IA},2}", 0.4, 0.6, 0.5, True),
            ("ia_a_d", r"A_{\mathrm{IA},d}", 0.4, 0.6, 0.5, True),
            ("ia_zpiv_1", r"z_{\mathrm{piv},1}", 0.0, 1.0, 0.62, False),
            ...
            ("ia_alphaz_2", r"\alpha_{z,2}", -5.0, 5.0, 0.0, False),
            ("ia_alphaz_d", r"\alpha_{z,d}", -5.0, 5.0, 0.0, False),
            ("lens0_b_2", r"b_2^0", 0.8, 1.2, 1.0, True),
            ("lens0_b_s", r"b_s^0", 0.8, 1.2, 1.0, True),
            ("lens0_mag_bias", r"s_{\mathrm{mag}}^0", 0.8, 1.2, 1.0, True),
            ("lens0_bias", r"b_0", 0.8, 3.0, 2.0, True),
            ("src0_delta_z", r"\delta z_{src0}", -0.05, 0.05, -0.001, True),
            ("lens0_delta_z", r"\delta z_{lens0}", -0.05, 0.05, 0.001, True),
        ]
