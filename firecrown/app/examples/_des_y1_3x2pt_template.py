...
def build_likelihood(params: NamedParameters) -> tuple[ConstGaussian, ModelingTools]:
    ...
    # Creates a LAI systematic. This is a systematic that is applied to all
    # weak-lensing probes. The `sacc_tracer` argument is used to identify the section
    # of the SACC file that this systematic will be applied to. In this case we want to
    # apply it to all weak-lensing probes, so we use the empty string.
    lai_systematic = wl.LinearAlignmentSystematic(sacc_tracer="")

    # Creating sources, each one maps to a specific section of a SACC file. In this
    # case src0, src1, src2 and src3 describe weak-lensing probes. The sources are
    # saved in a dictionary since they will be used by one or more two-point function.
    sources: dict[str, wl.WeakLensing | nc.NumberCounts] = {}

    for i in range(4):
        # Each weak-lensing section has its own multiplicative bias. Parameters reflect
        # this by using src{i}_ prefix.
        mbias = wl.MultiplicativeShearBias(sacc_tracer=f"src{i}")

        # We also include a photo-z shift bias (a constant shift in dndz). We also have
        # a different parameter for each bin, so here again we use the src{i}_ prefix.
        wl_pzshift = wl.PhotoZShift(sacc_tracer=f"src{i}")

        # Now we can finally create the weak-lensing source that will compute the
        # theoretical prediction for that section of the data, given the systematics.
        sources[f"src{i}"] = wl.WeakLensing(
            sacc_tracer=f"src{i}", systematics=[lai_systematic, mbias, wl_pzshift]
        )

    # Creating the number counting sources. There are five sources each one labeled by
    # lens{i}.
    for i in range(5):
    ...
    # Now that we have all sources we can instantiate all the two-point functions. The
    # weak-lensing sources have two "data types", for each one we create a new
    # two-point function.
    stats = {}
    ...
    # The following two-point function objects compute the cross correlations between
    # the weak-lensing and number count sources.
    for j in range(5):
        for i in range(4):
    ...
    # Here we instantiate the actual likelihood. The statistics argument carry the
    # order of the data/theory vector.
    likelihood = ConstGaussian(statistics=list(stats.values()))

    # We load the correct SACC file.
    sacc_file = params.get_string("sacc_file")
    # Translate environment variables, if needed.
    sacc_file = os.path.expandvars(sacc_file)
    sacc_data = load_sacc_data(sacc_file)

    # The read likelihood method is called passing the loaded SACC file, the two-point
    # functions will receive the appropriated sections of the SACC file and the sources
    # their respective dndz.
