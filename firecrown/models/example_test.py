...
cluster_mass_r = ClusterMassRich(hmd_200, hmf_200, proxy_params, pivot_mass=pivot_mass, pivot_redshift=pivot_redshift, hmf_args=hmf_args)
...
for i in range(len(z_bins)-1):
    for j in range(len(proxy_bins)-1):
        result = cluster_abundance_bin.compute_bin_N(ccl_cosmo, proxy_bins[j], proxy_bins[j+1], z_bins[i], z_bins[i+1])
        test.append(result)
print(test)
mass_bins = [13,14,15]
test = []
for i in range(len(z_bins)-1):
    for j in range(len(mass_bins)-1):
        result = cluster_abundance_bin_p.compute_bin_N(ccl_cosmo, mass_bins[j], mass_bins[j+1], z_bins[i], z_bins[i+1])
        test.append(result)
print(test)
...
for i in range(len(z_bins)-1):
    for j in range(len(proxy_bins)-1):
        result = cluster_mean_bin.compute_bin_logM(ccl_cosmo, proxy_bins[j], proxy_bins[j+1], z_bins[i], z_bins[i+1])
        test.append(result)
print(test)
