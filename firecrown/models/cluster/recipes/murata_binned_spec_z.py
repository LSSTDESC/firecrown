...
class MurataBinnedSpecZShearRecipe(ClusterRecipe):
    """Cluster recipe with Murata19 mass-richness and spec-zs.
    ...
    def get_theory_prediction(
        ...
        [npt.NDArray[np.float64], npt.NDArray[np.float64], tuple[float, float], tuple[float, float], float],
        npt.NDArray[np.float64],
    ]:
        """Get a callable that evaluates a cluster theory prediction.
        ...
        expected number of clusters.
        """
        def extra_kernel(mass, z, radius_limits):
            return 1.0
        for cluster_prop in ClusterProperty:
            include_prop = cluster_prop & average_on
            if not include_prop:
                continue
            if cluster_prop == ClusterProperty.SHEAR:
                def extra_kernel(mass, z, radius_limits):
                    return 1.0
            else:
                raise NotImplementedError(f"Average for {cluster_prop}.")
        ...
        def theory_prediction(
            ...
            return (
                cluster_theory.comoving_volume(z, sky_area=1.0)
                * cluster_theory.mass_function(mass, z)
                * self.redshift_distribution.distribution()
                * self.mass_distribution.distribution(mass, z, mass_proxy_limits)
                * extra_kernel(mass, z, radius_limits)
            )
    ...
    def get_function_to_integrate(
        ...
        def function_mapper(
            ...
            mass_proxy_low = extra_args[0]
            mass_proxy_high = extra_args[1]
            radius_low = extra_args[0]
            radius_high = extra_args[1]

            return prediction(mass, z, (mass_proxy_low, mass_proxy_high), (radius_low, radius_high))
    ...
    def evaluate_theory_prediction(
        ...
        self.integrator.extra_args = np.array([*this_bin.mass_proxy_edges, this_bin.radius_edges])
