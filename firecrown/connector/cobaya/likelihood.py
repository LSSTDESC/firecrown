...
class LikelihoodConnector(Likelihood):
    ...
    def initialize(self):
        ...
        if ext == ".yaml":
            ...
            analyses = set(self.data.keys()) - set(
            ...
            for analysis in analyses:
                ...
                self.likelihood.set_systematics(
                    self.data[analysis]["data"]["systematics"]
                )
                self.likelihood.set_statistics(
                    self.data[analysis]["data"]["statistics"]
                )
            ...
            if not hasattr(mod, "likelihood"):
                raise ValueError(
                    f"Firecrown initialization file {self.firecrownIni} does not define a likelihood."
                )

            self.likelihood = mod.likelihood
        else:
            raise ValueError(
                f"Unrecognized Firecrown initialization file {self.firecrownIni}."
            )
