...
def test_linear_bias_systematic_smoke():
    ...
    sys = LinearBiasSystematic(alphag="__alphag", alphaz="__alphaz", z_piv="__z_piv")
...
def test_magnification_bias_systematic_smoke():
    ...
    sys = MagnificationBiasSystematic(
        r_lim="__r_lim", sig_c="__sig_c", eta="__eta", z_c="__z_c", z_m="__z_m"
    )
    ...
    mag_bias = (
        ...
        * (
            19.0 / 25.5
            - 3 * 0.055 / z_bar
            + 1.5 * 0.055 * np.power(src.z_ / z_bar, 1.5) / z_bar
        )
    )
