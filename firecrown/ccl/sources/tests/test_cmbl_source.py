...
def cmbl_data():
    ...
    sacc_data.add_tracer(
        "Map", "trc1", quantity="cmb_convergence", spin=0, ell=ell, beam=None
    )
...
def test_cmbl_source_nosys(cmbl_data):
    src = CMBLSource(sacc_tracer=cmbl_data["sacc_tracer"], scale=0.5)
    src.read(cmbl_data["sacc_data"])

    src.render(cmbl_data["cosmo"], cmbl_data["params"], cmbl_data["systematics_dict"])
