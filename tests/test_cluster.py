...
def initialize_objects():
    ...
    cluster_mass_proxy = ClusterMassRich(
        hmd_200, hmf_200, proxy_params, pivot_mass, pivot_redshift, hmf_args
    )
    ...
    cluster_abundance_binned_Mproxy = ClusterAbundanceBinned(
        cluster_mass_proxy, cluster_z, sky_area
    )
    return [
        cluster_mass,
        cluster_z,
...
def test_cluster_mass_functions():
    ...
    assert cluster_mass.compute_mass_function(
        ccl_cosmo, logM, z
    ) == hmf_test.get_mass_function(ccl_cosmo, 10**logM, a)
    ...
    test_mass_intp_bin = cluster_mass_proxy.cluster_logM_intp_bin(
