...
from firecrown.models.cluster.recipes.murata_unbinned_spec_z import (
    MurataUnbinnedSpecZRecipe,
)
...
@pytest.fixture(name="murata_unbinned_spec_z")
def fixture_murata_unbinned_spec_z() -> MurataUnbinnedSpecZRecipe:
    cluster_recipe = MurataUnbinnedSpecZRecipe()
...
def test_murata_unbinned_spec_z_init():
    recipe = MurataUnbinnedSpecZRecipe()
    ...
    assert isinstance(recipe.mass_distribution, MurataUnbinned)
...
def test_get_theory_prediction_returns_value(
    ...
    murata_unbinned_spec_z: MurataUnbinnedSpecZRecipe,
):
    prediction = murata_unbinned_spec_z.get_theory_prediction(cluster_abundance)
    ...
    mass_proxy = np.linspace(0, 5, 2)
    sky_area = 360**2

    result = prediction(mass, z, mass_proxy, sky_area)
...
def test_get_theory_prediction_with_average_returns_value(
    ...
    prediction = murata_unbinned_spec_z.get_theory_prediction(
...
def test_get_theory_prediction_throws_with_nonimpl_average(
    ...
    with pytest.raises(NotImplementedError):
        _ = prediction(mass, z, mass_proxy, sky_area)
...
def test_get_function_to_integrate_returns_value(
    ...
    prediction = murata_unbinned_spec_z.get_theory_prediction(cluster_abundance)
    function_to_integrate = murata_unbinned_spec_z.get_function_to_integrate(prediction)

    assert function_to_integrate is not None
    assert callable(function_to_integrate)

    int_args = np.array([[13.0, 0.1, 0.1], [17.0, 1.0, 4.9]])
    extra_args = np.array([360**2])
...
def test_evaluates_theory_prediction_returns_value(
    ...
    prediction = murata_unbinned_spec_z.evaluate_theory_prediction(
