...
class TestAtLeastOneTracerHasHM:
    """Test class for the at_least_one_tracer_has_hm function."""
    ...
    @pytest.fixture
    def mock_tracer_without_hm(self):
    ...
    def test_both_tracers_have_hm(self, mock_tools, mock_tracer_with_hm):
    ...
    def test_only_first_tracer_has_hm(
        self, mock_tools, mock_tracer_with_hm, mock_tracer_without_hm
    ):
    ...
    def test_mixed_tracer_requires_shear(self, mock_tools):
    ...
    def test_profile_assertion_for_mixed_hm(self, mock_tools, mock_tracer_without_hm):
    ...
    def test_ia_bias_exponent_both_hm(self, mock_tools, mock_tracer_with_hm):
        ...
        with patch("pyccl.halos.halomod_Pk2D", return_value=mock_pk_1h), patch(
            ...
            # Check that from_function was called with ia_bias_exponent=2
            call_args = mock_from_func.call_args
            assert call_args is not None

            # The ia_bias_exponent should be in kwargs
            kwargs = call_args[1] if len(call_args) > 1 else {}
            # It might also be in args depending on how the function is called
            args = call_args[0] if len(call_args) > 0 else ()

            # Verify that ia_bias_exponent=2 is used (could be in args or kwargs)
            found_exponent = False
            if "ia_bias_exponent" in kwargs and kwargs["ia_bias_exponent"] == 2:
                found_exponent = True
            elif len(args) >= 4 and args[3] == 2:  # Depending on position
                found_exponent = True

            # Note: The exact verification depends on how pyccl.Pk2D.from_function is called
    ...
    def test_pk2d_addition_result(self, mock_tools, mock_tracer_with_hm):
