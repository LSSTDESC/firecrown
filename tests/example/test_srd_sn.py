...
@pytest.fixture(name="sn_srd_example")
def fixture_sn_srd_example(
    target_framework: Frameworks, use_absolute_path: bool, tmp_path: Path
) -> tuple[Path, Frameworks]:
    """Generate SN SRD example for all frameworks and path types."""
    ExampleSupernovaSRD(
...
def test_srd_sn_run(sn_srd_example):
    ...
    match framework:
        ...
        case Frameworks.NUMCOSMO:
            result = subprocess.run(
                ["numcosmo", "run", "test", "numcosmo_sn_srd.yaml"],
                cwd=output_path,
                capture_output=True,
                text=True,
                check=True,
