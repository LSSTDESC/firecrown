...
For background on the factory system, see [Two-Point Factory Basics](two_point_factory_basics.qmd). For loading SACC data, see [Loading SACC Data](two_point_sacc_loading.qmd). For information on defining systematics factories, see [Two-Point Generators](two_point_generators.qmd#sec-factories).
...
With this formalism, we can evaluate the likelihood exactly as in the [Loading SACC Data](two_point_sacc_loading.qmd) tutorial by defining filters to be very wide. Alternatively, by setting a restrictively small filtered range, we can remove data from the analysis. In the example below, we filter out all galaxy clustering data by using an extremely narrow range.
