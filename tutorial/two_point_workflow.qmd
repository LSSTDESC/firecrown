...
1. **[Loading SACC Data](two_point_sacc_loading.qmd)** — Extract metadata and measurements
...
### From SACC Files ([Tutorial](two_point_sacc_loading.qmd))
...
- Integrate with [[likelihood.factories|TwoPointExperiment]] for complete configuration management

**When to use:** Models with limited validity (e.g., excluding small scales where baryonic effects dominate, or large scales with poor statistics).

## Step 4: Compute Theory Predictions ([Tutorial](two_point_integration.qmd))

Control integration methods for harmonic-space calculations via [[utils|ClIntegrationOptions]]:

- **Limber approximation:** Fast but less accurate at large scales
- **FKEM methods:** More accurate but computationally expensive
...
| [Loading SACC](two_point_sacc_loading.qmd) | Extract from SACC files | Real data, existing simulations |
