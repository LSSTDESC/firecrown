...
To address this, we use a shared `GalaxyObservableModelParameters` class, which includes the redshift distribution, a generic `scale` parameter, and a `field` parameter specifying the power spectrum field to use.

This shared structure allows us to define a common set of systematics that can be applied across all source tracers.
For instance, the `PhotoZShift` object modifies the redshift distribution in a `GalaxyObservableModelParameters` object.

The subclasses `WeakLensingArgs` and `NumberCountsArgs` inherit from `GalaxyObservableModelParameters` and add additional fields specific to weak lensing and number counts, respectively.
...
class SourceGalaxySystematic {
   apply(self, tools: ModelingTools, tracer_arg: _GalaxyObservableModelParametersT) 
}
...
## Catalog of Systematics


class SourceSystematic
class SourceGalaxySystematic(SourceSystematic, Generic[_GalaxyObservableModelParametersT]):
...
class HMAlignmentSystematic
class LinearAlignmentSystematic
class MultiplicativeShearBias
class TattAlignmentSystematic
...
class ConstantMagnificationBiasSystematic
class LinearBiasSystematic
class MagnificationBiasSystematic
class PTNonLinearBiasSystematic
class SelectField
