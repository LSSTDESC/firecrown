...
Source tracer objects, such as `NumberCounts`, `WeakLensing`, and `CMBConvergence`, require configuration details, calibration inputs, and model parameters.
...
Galaxy source tracers share a common requirement: a redshift distribution (e.g., the galaxy redshift distribution).
...
The CMB lensing convergence source is not a `SourceGalaxy` subclass; it uses `CMBConvergenceArgs`, including a source redshift for the CMB last-scattering surface, and does not use the galaxy redshift-distribution argument hierarchy.
...
[^ptnlbs]: `firecrown.likelihood.number_counts.PTNonLinearBiasSystematic` is the full name of this class.
...
[^1]: `firecrown.likelihood.SourceSystematic` is the full name of this class.
...
[^2]: There are actually two classes named `PhotoZShift`: one is `firecrown.likelihood.weak_lensing.PhotoZShift`; the other is `firecrown.likelihood.number_counts.PhotoZShift`.
      They are both derived from the `firecrown.likelihood.SourceSystematic` base class.
...
[^other]: Firecrown currently includes the non-galaxy `CMBConvergence` source for CMB lensing convergence. Dedicated non-galaxy source systematics are still more limited than the galaxy `SourceGalaxySystematic` hierarchy described here.
...
```{python}
# Continuing from above...
import firecrown.updatable as params
...
for name, value in default_parameters.params.items():
  print(f"\t{name}: {value}")
```
