...
The systematic can be queried for the names of its parameters in a slightly roundabout fashion:

```{python}
import firecrown.likelihood.number_counts as nc
systematic = nc.MagnificationBiasSystematic(sacc_tracer="lens0")
print("The parameter names of this systematic are:")
for name in systematic.required_parameters().get_params_names():
    print(f"\t{name}")
```
...
for p in systematic._sampler_parameters:
  print(f"\tname: {p.name}  value: {p.value}")
```
...
But note that the contained sampler parameters have not been updated:
...
::: {.callout-important}
## How are the contained _sampler_parameters used?

The `SamplerParameters` contained in the systematic are not given a value when `update` is called.
How are they used?
:::
