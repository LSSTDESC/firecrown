...
The type used to represent a cosmology in Firecrown comes from CCL: [`pyccl.Cosmology`](https://ccl.readthedocs.io/en/latest/api/pyccl.cosmology.html).
...
```{.python}
class MyLikelihood(firecrown.likelihood.Likelihood):
    """A dummy sample likelihood class."""

    def read(sacc: sacc.SACC) -> None:
        """Read data from a SACC object"""
        ...

    def calculate_loglike(tools: ModelingTools) -> float:
        """Calculate the log-likelihood for the given modeling tools."""
        ...
```
...
Gaussian-related likelihoods are subclasses of `firecrown.likelihood.GaussFamily`.
