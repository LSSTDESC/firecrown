...
**Note:** This tutorial demonstrates generating two-point statistics from scratch. If you have existing data in a SACC file, see [Loading SACC Data](two_point_sacc_loading.qmd) for how to extract and use it.
...
# As of the current Firecrown version, we do not have any global systematics for number counts
...
The `TwoPointFactory` here uses default integration options. For information on controlling integration methods (Limber approximation vs. FKEM methods), see [Integration Methods](two_point_integration.qmd).
