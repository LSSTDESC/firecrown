...
In the tutorial found in the [Redshift Distributions tutorial](inferred_zdist_generators.html), we demonstrate how to use Firecrown to generate the redshift distribution for a given set of parameters. In this document, we demonstrate how to use the `InferredGalaxyZDist` object to generate the two-point statistics for the Large Synoptic Survey Telescope (LSST) using the redshift distribution from the LSST Science Requirements Document (SRD).

First we will generate all bins for the LSST year 1 redshift distribution:
...
In `firecrown.metadata` we have functions that compute all possible two-point statistics for a given set of bins. For example, below we compute all possible two-point statistics for the LSST year 1 redshift distribution:
...
The code above generates the following table of two-point statistics:
```{python}
# | code-fold: true
import pandas as pd
...
In order to generate the two-point statistics, we need to define the factories for the weak lensing and number counts systematics. The code below defines the factories for the weak lensing and number counts systematics:
...
Setting up the two-point statistics requires a cosmology and a set of parameters. The parameters necessary in our analysis depend on the two-point statistics and the systematics we are considering. In the code below, we extract the required parameters for the two-point statistics and create a `ParamsMap` object with the default values:
