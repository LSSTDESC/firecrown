...
A `Statistic` also has indices that are used to identify what blocks of covariance matrices in the `sacc.Sacc` object will be read.
A given `sacc.Sacc` object may contain information from observations in many bins, but only those indicated by the indices in a `Statistic` will be read.
...
The class `firecrown.likelihood.TwoPoint` is a statistic that represents a two-point function.
...
Currently, we have three implementations of `Source`: `NumberCounts`, `WeakLensing`, and `CMBConvergence`.
...
The `WeakLensing` source represents a weak lensing measurement; these measurements result from light emitted by source galaxies being lensed by the matter distribution in the Universe.
They are usually labeled as `src` sources.
The `CMBConvergence` source represents CMB lensing convergence, using the CMB last-scattering surface as the source plane.
It can be used in two-point statistics involving CMB convergence, including cross-correlations with galaxy observables.
...
CMB convergence support currently provides a source and factor., but it does not have the same `SourceGalaxySystematic` hierarchy used by `NumberCounts` and `WeakLensing`.
