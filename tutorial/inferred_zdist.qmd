...
This will be discussed in another tutorial.
...
two_point_lens0_lens0 = TwoPoint.from_metadata_cells(
    [lens0_lens0_cell],
...
The source factories will be discussed in another tutorial.

Depending on the theoretical model, the `TwoPoint` object may require additional parameters.
To obtain a list of available factories, one can use the following command:
...
We can use Firecrown's `ParamsMap` to obtain the required parameters for the theoretical model and their default values.
...
```{python}
# | label: fig-fz
# | fig-cap: Two-point correlation function for the bin pair (lens0, lens0).
