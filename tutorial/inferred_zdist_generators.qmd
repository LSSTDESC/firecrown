...
In the previous [tutorial](tomographic_bin.qmd), we explored how to use `TomographicBin` objects to represent redshift distributions for galaxies. 
...
In Firecrown this distribution is represented as an object of type `TomographicBin`[^1].
...
`ZDistLSSTSRD` provides these values as defaults and allows for greater flexibility when desired.
...
import numpy as np
import firecrown
from firecrown.generators import ZDistLSSTSRD
...
We use NumCosmo's `autoknots` algorithm to determine the number of knots to use in the spline interpolation.
This allows us to create the knots such that the interpolation error is less than the specified tolerance (here $10^{-5}$).[^4]

[^4]: The `autoknots` method is part of the NumCosmo package, which is a dependency of Firecrown.
See [the NumCosmo documentation](https://numcosmo.readthedocs.io/en/latest/reference/numcosmo-math/class.SplineCubicNotaknot.html) for more information.
...
# The data were not originally generated in a data frame convenient for
...
Next, using the same SRD prescriptions, we want to generate the `TomographicBin` objects representing @eq-Pi for a specific binning, and using a specific resolution parameter $\sigma_z$.
...
[^3]: Note that we are making use of several module-level constants in the `firecrown.generators` module, namely `Y1_LENS_BINS`, `Y10_LENS_BINS`, `Y1_SOURCE_BINS`, and `Y10_SOURCE_BINS`, which are dictionaries that contain the bin edges and the resolution parameter $\sigma_z$ for the bins.
...
# We create two TomographicBin objects, one for Y1 and one for Y10.
...
assert isinstance(Pz_lens0_y1, TomographicBin)
assert isinstance(Pz_lens0_y10, TomographicBin)
assert isinstance(Pz_source0_y1, TomographicBin)
assert isinstance(Pz_source0_y10, TomographicBin)
```

The plot of the $\textrm{d}N/\textrm{d}z$ distributions in these `TomographicBin` objects is shown in @fig-inferred-dist.
...
# This time we generate the data frame using a different technique.
...
The plot of the $\textrm{d}N/\textrm{d}z$ distributions in these `TomographicBin` objects is shown in @fig-inferred-dist-all.
...
The `last_z` parameter specifies the maximum photometric redshift to consider when generating bins.
```{python}
...
# | fig-cap: Source bin edges for LSST Year 1 using the formulation from the SRD.
# | fig-cap-location: margin
# | code-fold: true
...
The red dashed lines represent the bin edges generated using the true redshift distribution, while the blue dotted lines represent the bin edges generated using the photometric redshift distribution.
The table below shows the percentage difference in the number of galaxies in each bin between the two methods.
```{python}
...
Any facility that can create the $z$ and $\textrm{d}N/\textrm{d}z$  arrays can be used to create an `TomographicBin` object.
Code that uses the `TomographicBin` objects does not depend on how they were created.
