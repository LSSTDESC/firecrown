...
This halo profile would be included in the modeling tools so that the same halo profile is used consistently wherever one is needed.
If one needs a different way to calculate power spectra, for example, perturbation theory or halo models, then those tools would also be available with the cosmology in the modeling tools.
...
The theoretical prediction associated with the data vector is computed by a *statatistic* array.
