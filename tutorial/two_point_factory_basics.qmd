...
2. **Delegates to source factories** (e.g., [[likelihood.weak_lensing|WeakLensingFactory]], [[likelihood.number_counts|NumberCountsFactory]], and [[likelihood|CMBConvergenceFactory]]) based on measurement types
...
- [[metadata_types|CMB.CONVERGENCE]] → [[likelihood|CMBConvergenceFactory]]

For analyses containing CMB lensing convergence measurements, include a `cmb_factories` entry in the `TwoPointFactory` configuration. Analyses with only galaxy counts and galaxy shear do not need to configure a CMB factory.
...
This example shows the full workflow starting from generated metadata. For details on generating the LSST Year 1 bins and metadata, see [Two-Point Generators](two_point_generators.qmd). In this example we use the 3x2pt bin pair selection logic from [Bin Pair Selectors](two_point_bin_selectors.qmd).
