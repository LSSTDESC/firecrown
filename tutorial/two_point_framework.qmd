...
The framework then separates measurement types by data layout: [[metadata_types|TwoPointHarmonic]] for harmonic-space measurements (e.g., as a function of multipole moments $\ell$) and [[metadata_types|TwoPointReal]] for real-space measurements (e.g., as a function of angular separation $\theta$). 
...
At the top of the hierarchy is the [[data_types|TwoPointMeasurement]], which includes the data layout ([[metadata_types|TwoPointHarmonic]] or [[metadata_types|TwoPointReal]]) and the associated measured values, either real data or mock realizations. 
...
To compute theoretical predictions or build likelihoods, a [[likelihood.two_point|TwoPoint]] object must be instantiated. 
...
Multiple [[likelihood.two_point|TwoPoint]] instances can be constructed from the same layout with different model choices. While this can be done manually, the [[likelihood.two_point|TwoPointFactory]] automates the creation of [[likelihood.two_point|TwoPoint]] objects from either layout or measurement objects. See [Two-Point Factories](#sec-two-point-factories) for details.
...
Additionally, it contains a set of [[metadata_types|Measurement]] types (e.g., [[metadata_types|Galaxies.COUNTS]], [[metadata_types|Galaxies.SHEAR_T]], [[metadata_types|CMB.CONVERGENCE]]) indicating which observables were measured in this bin. 
...
A given bin (e.g., a sample of galaxies) may be involved in multiple different observables, such as number counts and shear, and [[metadata_types|TwoPointXY]] allows each measurement pairing to be represented without duplicating bin information.
...
The previous objects ([[metadata_types|InferredGalaxyZDist]], [[metadata_types|TwoPointXY]], [[metadata_types|TwoPointHarmonic]], [[metadata_types|TwoPointReal]]) define the structure and layout of a two-point measurement, what is being measured and how, but do not include actual data. 
...
- [[metadata_types|Galaxy.COUNTS]] → [[likelihood.number_counts|NumberCountsFactory]]
- [[metadata_types|Galaxy.Galaxy.SHEAR_E]] `|` [[metadata_types|Galaxy.Galaxy.SHEAR_T]] `| ...` → [[likelihood.weak_lensing|WeakLensingFactory]]
