...
- To initialize a [[likelihood|TwoPoint]] object, which encapsulates all modeling choices and can be used to compute theoretical predictions and/or contribute to a likelihood.

A [[likelihood|TwoPoint]] object can be constructed from either a layout ([[metadata_types|TwoPointHarmonic]] or [[metadata_types|TwoPointReal]]) or a measurement ([[data_types|TwoPointMeasurement]]). In both cases, it computes a [[data_types|TheoryVector]]. If the object includes data (i.e., initialized from a [[data_types|TwoPointMeasurement]]), it also contains a [[data_types|DataVector]]. As a subclass of Firecrown's [[likelihood|Statistic]], a [[likelihood|TwoPoint]] can be included in a [[likelihood|Likelihood]]; see [Introduction](intro_article.qmd) for further details.

To simplify the construction of [[likelihood|TwoPoint]] objects from layouts, Firecrown provides the [[likelihood|TwoPointFactory]]. This factory automates source creation based on metadata found in the layout description. Specifically, it inspects the measurement types in each [[metadata_types|TwoPointXY]] (i.e., the [[metadata_types|TwoPointXY.x_measurement]] and [[metadata_types|TwoPointXY.y_measurement]] fields) and delegates to appropriate source factories—such as [[likelihood.weak_lensing|WeakLensingFactory]] or [[likelihood.number_counts|NumberCountsFactory]].
...
The [[likelihood|TwoPointFactory]] can hold multiple instances of the same source factory, each tied to a different [[metadata_types|TypeSource]]. This allows distinct modeling choices for different subpopulations (e.g., red vs. blue galaxies), even when the measurement type is the same. By default, both basic bins and factories use [[metadata_types|TypeSource.DEFAULT]], so in simpler analyses the [[metadata_types|TypeSource]] distinction can be ignored.
