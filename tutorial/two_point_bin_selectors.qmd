...
| Requirement | Selector Combination | Notes |
|---------------------|---------------------|-----|
| All (Source, Lens) pairs | [[metadata_types|SourceLensBinPairSelector]] | Fundamental building block |
| Galaxy-galaxy lensing<br>(with naming convention) | `SourceLensBinPairSelector() &`<br>`CrossNameDiffBinPairSelector(`<br>`  neighbors_diff=[0, 1])` | Assumes index ∝ redshift |
| Nearby bins<br>(with naming convention) | `AutoNameDiffBinPairSelector(`<br>`  neighbors_diff=[0, 1, -1])` | Limits index differences |
| Same data source | `TypeSourceBinPairSelector(`<br>`  type_source=...)` | Filter by provenance |
| Standard 3×2pt<br>(with naming convention) | `ThreeTwoBinPairSelector()` | Pre-built composite |
| Custom high-S/N | `ThreeTwoBinPairSelector(`<br>`  source_dist=2, lens_dist=2,`<br>`  source_lens_dist=1)` | Conservative distances |
