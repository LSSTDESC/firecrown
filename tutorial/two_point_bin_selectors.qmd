...
redshift_constrained_selector = SourceLensBinPairSelector() & NameDiffBinPairSelector(
    ...
    neighbors_diff=[0, 1],   # Lens index ≤ source index + 1
)
...
|--------------|-------|---------|----------|
| `same_name_prefix=True`<br>`neighbors_diff=1` | (bin0, bin1)<br>(bin1, bin0)<br>(bin2, bin2) | (bin0, bin2)<br>(src0, bin0) | Adjacent bins, same type |
...
class MyCustomSelector(CompositeSelector):
    ...
    def model_post_init(self, _: Any, /) -> None:
        self._impl = (
            SourceLensBinPairSelector()
            & CrossNameDiffBinPairSelector(
                neighbors_diff=list(range(0, self.max_separation + 1))
            )
        )
```
...
default_3x2pt = ThreeTwoBinPairSelector(
    source_dist=5,       # Cosmic shear: |source_i - source_j| < 5
    lens_dist=5,         # Clustering: |lens_i - lens_j| < 5  
...
- `source_dist`: For cosmic shear, allows source bins with indices differing by up to this amount
  - Range: `[-source_dist, source_dist)` (zero included)
  - Larger values → more cross-redshift correlations → more data, but weaker signals
  
- `lens_dist`: For galaxy clustering, same logic for lens bins
  - Range: `[-lens_dist, lens_dist)`
...
print("Note: diff = source_idx - lens_idx should be positive (lens at lower/equal z)\n")
display(Markdown(df.to_markdown(index=False)))
```
