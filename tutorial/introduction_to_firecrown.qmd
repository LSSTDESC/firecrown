...
-   Modeling tools
-   Likelihoods
-   Statistics (for Gaussian-family likelihoods)
-   Sources (for two-point statistics)
-   Systematics

## High-level Firecrown concepts

-   Cosmology (`pyccl.Cosmology`): represents a parameterized cosmological model.
-   Modeling tools (`firecrown.likelihood.ModelingTools`): associates a set of theoretical models for physical effects with a cosmology.
...
-   Likelihoods: calculates the probability of observing some given (presumably DESC) *data*, based on a certain set of parameters used in predictions of that data.
    -   `calculate_loglike(tools) -> float`

## Building blocks for likelihoods

-   Statistics: provide access to observations (a *data vector*) and to calculate predictions (a *theory vector*) based on a set of parameters (a *cosmology*).
    -   `get_data_vector() -> DataVector`
    -   `compute_theory_vector(tools) -> TheoryVector`
-   Sources:
-   Systematics
