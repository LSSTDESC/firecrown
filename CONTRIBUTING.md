...
firecrown ships — the package, its command line tools and the examples. That is
broader than what firecrown imports: `compilers` is there for the two-phase
`cosmosis-standard-library` installation, `pip` because cobaya is not packaged
on conda-forge and has to come from PyPI to complete the environment, and
`astropy`, `camb` and `isitgr` because the examples need them.

`devenv` is what developing, testing and documenting needs — the link checker's
`bs4` and `requests` live here, since `make docs-linkcheck` is the only thing
that runs it.

`workarounds` is packages firecrown does not import and does not need present,
but has to constrain anyway, either to route around an upstream bug or because
whatever pulls them in does not bound them itself. What separates it from
`runtime` is that firecrown needs those present, and merely needs these bounded.
Give every `workarounds` entry a `note` saying what it works around, so the
constraint can be retired when the upstream fix lands instead of outliving the
bug.

One trap when moving an entry between groups: the `pip` field is consulted for
`runtime` but not for `devenv`, so a conda-only package that was harmless in
`devenv` will be written into `pyproject.toml` once it moves. Give it
`pip: false` — `compilers`, for one, is a 0.0.0 placeholder on PyPI, not the
conda-forge metapackage.

`dependencies-validated.yaml`, the version ranges the committed lockfiles
resolve to, belongs to a later stage and is regenerated by `make conda-lock`,
right after the lockfiles it reads. It is generated, but committed and
reviewed: changes to the pins should be legible in a pull request rather than
buried in a lockfile regeneration.

The ordering matters, because the manifest generates the environment that the
lockfiles are solved from, and the pins are read back out of those lockfiles:

```
dependencies.yaml -> environment.yml -> lockfiles -> dependencies-validated.yaml
